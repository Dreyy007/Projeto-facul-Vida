...
// Sugestões de especialidade por perfil
const especialidadesPorTipo = {
  estagiario: ['Psicologia Clínica', 'Psicologia Infantil', 'Neuropsicologia', 'Psicologia Organizacional', 'Psicologia Escolar', 'Avaliação Psicológica', 'Terapia Cognitivo-Comportamental'],
  coordenador: ['Coordenação Clínica', 'Supervisão de Estágio', 'Gestão em Saúde Mental'],
...
export default function Usuarios() {
  ...
  return (
    <div className="page">
      ...
      {codigoCriado && (
        <div style={{ background: 'var(--sbg)', border: '1px solid var(--success)', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
      ...
      <div className="card">
        <div className="card-body">
          <table className="tbl">
            <thead>
              <tr><th>Nome</th><th>E-mail</th><th>Perfil</th><th>Especialidade</th><th>Código</th><th>CRP/CRM</th><th>Status</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {usuarios.map(u => {
                ...
                return (
                  <tr key={u.id}>
                    ...
                    <td style={{ fontSize: 12, color: u.especialidade ? 'var(--text)' : 'var(--muted)' }}>
                      {u.especialidade || '—'}
                    </td>
                    <td>
                      {u.codigo
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal" style={{ maxWidth: 580 }}>
            ...
            <div className="form-grid">
              ...
              {/* Especialidade — aparece para todos os perfis */}
              <div className="fld" style={{ gridColumn: '1/-1' }}>
                <label>Especialidade</label>
                <input
                  value={form.especialidade}
                  onChange={e => setForm({...form, especialidade: e.target.value})}
                  placeholder={form.tipo === 'estagiario' ? 'Ex: Psicologia Infantil, Neuropsicologia...' : 'Ex: Coordenação Clínica, Gestão em Saúde...'}
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                    {sugestoes.map(s => (
                      <button key={s} type="button"
                        onClick={() => setForm({...form, especialidade: s})}
                        style={{
                          padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none',
                          background: form.especialidade === s ? 'var(--p)' : 'var(--p3)',
                          color: form.especialidade === s ? '#fff' : 'var(--p)',
                          transition: '.15s'
                        }}>
                        {s}
                      </button>
              ...
              {/* CRP/CRM se estagiário */}
              {form.tipo === 'estagiario' && (
                <div className="fld"><label>CRP / CRM</label>
                  <input value={form.crp_crm} onChange={e => setForm({...form, crp_crm: e.target.value})} placeholder="Ex: CRP 06/12345"/>
                </div>
              )}
